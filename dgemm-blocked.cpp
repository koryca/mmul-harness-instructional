...
void square_dgemm_blocked(int n, int block_size, double* A, double* B, double* C) 
{
   ...
   double * Alocal = (double*) malloc(block_size * n * sizeof(double));
   double * Blocal = (double*) malloc(block_size * n * sizeof(double));
   double * Clocal = (double*) malloc(block_size * n * sizeof(double));
         ...
         for(int k=0; k<n; k+=block_size){ 
            ...
            for(int ia = i; ia < i + block_size; ia++){
               ...
               std::cout << "Alocal at copy: " << Alocal[ia + k * block_size] 
                      << " A at copy: " << A[ia + k * block_size] 
                      << " A[" << ia << "][" << k * block_size << "]" << std::endl;
               // }
            }
            //copy B
            for(int kb = k; kb < k + block_size; kb++){
               ...
               std::cout << "Blocal at copy: " << Blocal[kb + j * block_size] 
                      << " B at copy: " << B[kb + j* block_size] 
                      << " B[" << kb << "][" << j * block_size << "]"<< std::endl;
               // }
            }  
