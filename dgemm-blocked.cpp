...
void square_dgemm_blocked(int n, int block_size, double* A, double* B, double* C) 
{
   ...
   for (int i=0; i<n; i+=block_size){
      for (int j=0; j<n; j+=block_size){ //same as i
         memcpy((void *)Clocal, (const void *)C, sizeof(double)*block_size);
         for(int k=0; k<n; k+=block_size){ // same as i
            memcpy((void *)Alocal, (const void *)A, sizeof(double)*block_size);
            memcpy((void *)Blocal, (const void *)B, sizeof(double)*block_size);
            ...
            memcpy((void *)C, (const void *)Clocal, sizeof(double)*block_size);
         }
      }
   }
